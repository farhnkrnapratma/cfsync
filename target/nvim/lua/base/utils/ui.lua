...
function M.set_indent()
    ...
    if input_avail then
        ...
        utils.notify(
            string.format("indent=%d %s", indent, vim.bo.expandtab and "expandtab" or "noexpandtab")
        )
    end
end
...
function M.toggle_buffer_autoformat(bufnr)
    ...
    utils.notify(
        string.format("Buffer autoformatting %s", bool2str(vim.b[bufnr].autoformat_enabled))
    )
end
